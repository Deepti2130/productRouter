...
export class ProductService {
  ...
  POST_URL:string = `${environment.baseUrl}/product.json`
  // Id_URL:string =`${environment.baseUrl}/product`
  ...
  return this._httpclient.get(this.POST_URL)
        .pipe(
          ...
          for(const key in data){
            productArr.push({...data[key],id : key})
          }
          return productArr
          })
        )
  ...
  fetchproduct(id:string):Observable<any>{
    let url = `${this.BASE_URL}/product/${id}.json`
    return this._httpclient.get(url)
   }


     postproduct(postobj:Iproduct):Observable<any>{
    return this._httpclient.post(this.POST_URL,postobj)
  }

  updatedproduct(product:Iproduct):Observable<any>{
    let updateurl = `${this.BASE_URL}/product/${product.id}.json`
    return this._httpclient.patch(updateurl,product)

   }

     Removeproduct(id:string):Observable<any>{

        let deleteurl = `${this.BASE_URL}/product/${id}.json`
